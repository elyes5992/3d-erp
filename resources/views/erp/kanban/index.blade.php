...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <h2 class="text-xl sm:text-2xl font-bold text-white">Project Board</h2>
    <div class="flex gap-2 bg-slate-800 p-1 rounded-lg">
        <a href="{{ route('erp.products.index') }}" class="px-3 py-2 text-slate-400 hover:text-white text-sm rounded flex items-center gap-2">
            <i class="fa-solid fa-list"></i><span class="hidden sm:inline">List</span>
        </a>
        <span class="px-3 py-2 bg-slate-700 text-white text-sm rounded shadow flex items-center gap-2">
            <i class="fa-solid fa-columns"></i><span class="hidden sm:inline">Board</span>
        </span>
    </div>
</div>
...
<div class="flex gap-3 sm:gap-4 lg:gap-6 overflow-x-auto pb-10 items-start h-[calc(100vh-200px)] -mx-4 sm:mx-0 px-4 sm:px-0">

    @foreach($columns as $status)
    <!-- COLUMN -->
    <div class="w-72 sm:w-80 flex-shrink-0 flex flex-col h-full">
        ...
        <div class="flex items-center justify-between mb-3 px-1">
            ...
            <button class="text-slate-600 hover:text-white transition p-1"><i class="fa-solid fa-plus"></i></button>
        </div>
        ...
        <div class="kanban-col flex-1 bg-slate-900/30 rounded-lg p-2 space-y-3 overflow-y-auto custom-scrollbar border border-transparent hover:border-slate-800 transition"
            ...
            <!-- CARD -->
            <div class="kanban-card group bg-slate-800 hover:bg-slate-750 p-3 rounded shadow-sm border border-slate-700 cursor-grab active:cursor-grabbing relative touch-manipulation"
                data-id="{{ $product->id }}">

                <!-- Card Title -->
                <div class="text-slate-200 font-medium text-sm mb-2 break-words">{{ $product->name }}</div>

                <!-- Card Footer (Category & Actions) -->
                <div class="flex justify-between items-center mt-2">
                    <span class="text-[10px] text-slate-500 bg-slate-900 px-1.5 py-0.5 rounded border border-slate-700 truncate max-w-[70%]">
                        {{ $product->category->name ?? 'No Category' }}
                    </span>

                    @if($product->pinterest_url)
                    <a href="{{ $product->pinterest_url }}" target="_blank" class="text-slate-500 hover:text-[#E60023] p-1 -m-1">
