...
<div class="mb-6">
    <button onclick="openCreateModal()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2">
        <i class="fa-solid fa-plus"></i><span class="hidden sm:inline">New Category</span>
    </button>
</div>
        ...
        <table class="w-full text-left text-sm text-slate-400">
            <thead class="bg-slate-900/50 text-slate-200 uppercase text-xs font-semibold">
                <tr>
                    ...
                    <th class="px-4 sm:px-6 py-4">Designs Count</th>
            ...
            <tbody class="divide-y divide-slate-700">
                ...
                <tr class="hover:bg-slate-700/50 transition">
                    <td class="px-4 sm:px-6 py-4 font-medium text-white">{{ $category->name }}</td>
                    <td class="px-4 sm:px-6 py-4">
                        <span class="bg-slate-700 text-white px-2 py-1 rounded text-xs whitespace-nowrap">{{ $category->products_count ?? 0 }} Designs</span>
                    </td>
                    <td class="px-4 sm:px-6 py-4 text-right space-x-2">
                        <button onclick="openEditModal({{ $category }})" class="text-indigo-400 hover:text-indigo-300 transition p-1"><i class="fa-solid fa-pen"></i></button>
                        <form action="{{ route('erp.categories.destroy', $category->id) }}" method="POST" onsubmit="confirmDelete(event)" class="inline">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-400 hover:text-red-300 transition p-1"><i class="fa-solid fa-trash"></i></button>
                        </form>
                    </td>
                </tr>
                @endforeach
