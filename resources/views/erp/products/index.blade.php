...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    <div class="bg-slate-800 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition group border border-slate-700 flex flex-col h-full">
        <!-- Status Bar -->
        <div class="h-2 w-full flex-shrink-0 {{ $product->status == 'Printing' ? 'bg-blue-500' : ($product->status == 'Done' ? 'bg-green-500' : 'bg-slate-600') }}"></div>
        ...
        <div class="p-5 flex flex-col flex-grow">
            ...
            <div class="flex items-center justify-between mt-auto pt-4 border-t border-slate-700/50">
                <span class="text-xs px-2 py-1 rounded border {{ $product->status == 'Printing' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : ($product->status == 'Done' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-slate-700 text-slate-400 border-slate-600') }}">
                    {{ $product->status }}
                </span>
                
                @if($product->pinterest_url)
                    <a href="{{ $product->pinterest_url }}" target="_blank" class="text-slate-500 hover:text-red-400 transition">
        ...
        <form id="productForm" action="{{ route('erp.products.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                     ...
                     <select name="status" id="p_status" class="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white focus:outline-none focus:border-indigo-500">
...
<script>
    function openCreateModal() {
        ...
        document.getElementById('p_status').value = 'Idea';
    ...
    function openEditModal(product) {
        ...
        // Construct the update URL: BASE_URL + /products/ + id
        // Using relative path
